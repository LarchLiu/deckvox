...
export async function parseWorkflowStreamAndReturnOutputs(url: string, input: string, token: string): Promise<any | undefined> {
  ...
  try {
    ...
    let workflowFinishedOutputs // Variable to store the outputs
    ...
    while (true) {
      ...
      if (done) {
        ...
        return workflowFinishedOutputs
      }
...
const slaideHubGithubRepo = runtimeConfig.slaideHubGithubRepo || ''
const githubToken = runtimeConfig.githubToken || ''
const platform = runtimeConfig.deployPlatform || 'cloudflare' // cloudflare, vercel, netlify, local
// GitHub API 配置
...
export async function getGithubFiles(path: string, branch: string = 'main'): Promise<GithubFiles[]> {
  try {
    const response = await githubRequest<GithubFiles[]>(
      `/contents/${path}?ref=${branch}`,
      'GET',
    )

    if (!Array.isArray(response)) {
      throw new TypeError('Invalid response from GitHub API: expected array')
    }
